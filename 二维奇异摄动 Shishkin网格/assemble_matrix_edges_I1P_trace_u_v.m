...
% number_of_local_basis_trial=size(Tb_uu_trial,1);     %在单个单元上trial局部基函数的个数
...
%     end
    ...
    vertices_right=P(:,T((1:4),T_right));     % 在边的左侧单元的顶点
    %% 
...
%      vertices_ww = sort(vertices_www, 'ascend'); 
        for alpha=1:number_of_local_basis_trial 
            for beta=1:number_of_local_basis_test 
                ...
                A(Tb_test(beta,T_left),Eb(4+alpha,current_edge))=A(Tb_test(beta,T_left),Eb(4+alpha,current_edge)) +tau*int_value; % 同一三角形项的贡献
                ...
                A(Tb_test(beta,T_right),Eb(4+alpha,current_edge))=A(Tb_test(beta,T_right),Eb(4+alpha,current_edge)) +tau*int_value; % 同一三角形项的贡献
