...
function A=assemble_matrix_edges_I1P_uD_v(coe_fun2,P,T,Tb_test,Eb,matrixsize2,basis_type_u_test,para,tau)
...
%          vertices_right=P(:,T((1:4),T_right));   %边右侧单元的顶点坐标
        for beta=1:number_of_local_basis_test 
                ...
                int_value=tau*Gauss_quadrature_2D_volume_test_uD_v(coe_fun2,Gauss_nodes,Gauss_weights,vertices_left,basis_type_u_test,beta,0,0);
                A(Tb_test(beta,T_left),1)=A(Tb_test(beta,T_left),1) +int_value; % 同一三角形项的贡献
