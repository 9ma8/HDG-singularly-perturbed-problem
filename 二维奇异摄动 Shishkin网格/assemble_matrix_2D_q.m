...
function A=assemble_matrix_2D_q(coe_fun,P,T,Tb_q_trial,Tb_q_test,matrixsize1,matrixsize2,basis_type_q_trial,der_x_trial,der_y_trial,basis_type_q_test,der_x_test,der_y_test,order_Gauss)
%===创建稀疏矩阵并定义参数
number_of_elements=size(T,2); %单元的个数
number_of_local_basis_trial=size(Tb_q_trial,1);    %在单个单元上trial局部基函数的次数
number_of_local_basis_test=size(Tb_q_test,1);   %返回行数
...
%             int_value=0;
            ...
            int_value=J*Gauss_quadrature_2D_volume_q_trial_test(coe_fun,Gauss_nodes,Gauss_weights,vertices,basis_type_q_trial,alpha,der_x_trial,der_y_trial,basis_type_q_test,beta,der_x_test,der_y_test);
            A(Tb_q_test(beta,n),Tb_q_trial(alpha,n))=A(Tb_q_test(beta,n),Tb_q_trial(alpha,n))+int_value;
