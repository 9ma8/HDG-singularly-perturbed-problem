...
function [error_L2,error_Lq,error_DG]=compute_supercloseness_errors_HDG1(u,q1,q2,u1,uex,pex,qex,uex1,para,method,geo,tau,data,varepsilon)
...
elseif para.basis_type_trace_u==101
    number_of_local_basis_testt=2;
       ...
       if (T_right~=-1)
          ...
          u_app_plus2=Eb((1:number_of_local_basis_testt),current_edge);     % 获取当前单元上的数值解
...
%                 计算左侧三角形上的插值函数值
                uapp_plus2=uapp_plus2 +((u_app_n1(alpha)-u_app_plus1(alpha))*FE_local_basis_2D(Gauss_nodes(1,k),Gauss_nodes(2,k),vertices_plus,para.basis_type_u,alpha,0,0)).^2;
            end
            for beta=1:number_of_local_basis_testt
                if Gauss_nodes(1,1)-Gauss_nodes(1,2)~=0

                  uapp_plus3=uapp_plus3 +((u_app_plus3(beta)-uapp_plus(beta))*FE_local_basis_1D_local(Gauss_nodes(1,k),vertices1(1,:),para.basis_type_trace_u,beta,0)).^2;
%                  int_exact2= int_exact2+(u_app_plus(alpha)-u_exact_nm(alpha))*FE_local_basis_2D(Gauss_nodes(1,k),Gauss_nodes(2,k),vertices_plus,para.basis_type,alpha,0,0);
               elseif Gauss_nodes(1,1)-Gauss_nodes(1,2)==0
                   uapp_plus3=uapp_plus3 +((u_app_plus3(beta)-uapp_plus(beta))*FE_local_basis_1D_local(Gauss_nodes(2,k),vertices1(2,:),para.basis_type_trace_u,beta,0)).^2;
               end
            end    
        end
            int_square_plus2=int_square_plus2 +Gauss_weights(k)*(tau-0.5*data.b1(Gauss_nodes(1,k),Gauss_nodes(2,k))*normal(1)-0.5*data.b2(Gauss_nodes(1,k),Gauss_nodes(2,k))*normal(2))*uapp_plus2;
%             计算右侧三角形上的积分项
            int_square_minus2=int_square_minus2 +Gauss_weights(k)*(tau-0.5*data.b1(Gauss_nodes(1,k),Gauss_nodes(2,k))*normal(1)-0.5*data.b2(Gauss_nodes(1,k),Gauss_nodes(2,k))*normal(2))*uapp_plus3;
