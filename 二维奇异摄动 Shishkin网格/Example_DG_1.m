...
outputFile = fopen('supercloseness1.txt', 'w');
...
%       fprintf(outputFile, '%d\t%.5e\t%.5e\t%.5e\t%s\t%s\n', N(i), error_L2(i), error_Lq(i), error_DG(i), 'N/A', 'N/A'); 
else
...
%       fprintf(outputFile, '%d\t\t%.5e\t\t%.5e\t\t%.5e\t\t%.5f\t\t%.5f\t\t%.5f\n', N(i), error_L2(i), error_Lq(i), error_DG(i), convergence_orders_L2(i-1), convergence_orders_Lq(i-1), convergence_orders_DG(i-1));
end
end
